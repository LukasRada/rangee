...
describe("Rangee", () => {
  test("serialize / deserialize", () => {
    ...
    expect(serializedResult).toStrictEqual(
      `{"s":{"s":"body>div>h1","c":0,"o":0},"e":{"s":"body>div>h1","c":0,"o":14}}`
    );
    expect(compressedResult).toStrictEqual(
      new Uint8Array([
        55, 130, 32, 206, 32, 92, 161, 146, 4, 96, 123, 0, 152, 19, 192, 124,
        200, 37, 128, 221, 208, 11, 1, 24, 64, 6, 132, 1, 140, 160, 1, 140, 197,
        168, 23, 204, 129, 76, 161, 138, 4, 80, 219, 60, 141, 34, 235, 105, 68,
        32, 5, 158, 189, 32, 0, 0,
      ])
    );
  });
  ...
  test("atomic serialize / atomic deserialize - multiple nodes", () => {
    ...
    expect(compressedResult).toStrictEqual(
      new Uint8Array([
        55, 130, 32, 206, 32, 92, 161, 146, 4, 96, 123, 0, 152, 19, 192, 124,
        200, 37, 128, 221, 208, 11, 1, 25, 211, 0, 7, 1, 12, 3, 177, 0, 26, 16,
        6, 50, 128, 6, 59, 18, 144, 129, 124, 232, 20, 202, 24, 160, 66, 131,
        54, 60, 68, 72, 86, 167, 81, 164, 22, 32, 218, 64, 14, 193, 195, 128,
        31, 126, 209, 192, 10, 70, 147, 46, 2, 196, 201, 84, 137, 64, 11, 190,
        0, 180, 136, 1, 155, 153, 58, 148, 183, 0, 20, 0, 152, 2, 82, 208, 108,
        213, 179, 46, 33, 120, 109, 132, 17, 209, 23, 215, 18, 53, 48, 182, 181,
        183, 178, 115, 112, 246, 149, 151, 144, 5, 102, 82, 0, 0,
      ])
    );
  });
