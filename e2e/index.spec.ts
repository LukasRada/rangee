...
test('should select whole heading and serialize with Rangee', async ({ page }) => {
...
test('should select text across few elements and serialize with Rangee', async ({ page }) => {
    await page.evaluate(() => {
    ...
    const button = await page.waitForSelector('#get');
    await button.click();
    const expectedTexts = ['omplex HTML Page', 'Header section with some text.', 'Sec', 'highlighted text'];
    const received = await page.evaluate(() => Array.from(document.querySelectorAll('mark')).map(el => el.textContent), []);
